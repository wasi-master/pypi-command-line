    ...
    if requests_size:
        ...
        for n, response in enumerate(session.cache.values()):
            table.add_row(
                f"{n}.",
                response.url,
                humanize.naturaltime(response.created_at),
                humanize.naturaltime(response.expires),
            )
        console.print(table)
    ...
    if minimal_output:
        ...
        if show_installed_version:
            ...
            try:
                ...
                version_info = (
                    f" [dark_orange](Installed Version: {installed_version})[/]"
                )
